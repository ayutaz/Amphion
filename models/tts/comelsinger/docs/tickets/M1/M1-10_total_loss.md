...
**ゴール**: `L_CL = lambda_scl * L_SCL + lambda_fcl * L_FCL`、`L_total = lambda_cl * L_CL + lambda_svt * L_SVT + lambda_mask * L_mask` を計算し、スカラーテンソルと各成分の辞書を返す。
...
- デフォルト重みで `total == 0.5*(1.0*l_scl + 0.1*l_fcl) + 0.5*l_svt + 0.3*l_mask`（atol=1e-6）
...
- `L_CL = lambda_scl * L_SCL + lambda_fcl * L_FCL` という形式は要件定義書12確定値（lambda_scl=1.0, lambda_fcl=0.1）に基づく
- `lambda_scl=1.0` と `lambda_fcl=0.1` の非対称な重みは SCL を支配的にする設計であり、FCLが過大にならないよう調整されている

### 5.2 レビュー項目

- [ ] 要件定義書12確定値 `λ_SCL=1.0, λ_FCL=0.1, λ_CL=0.5, λ_SVT=0.5, λ_mask=0.3` とデフォルト引数が一致しているか
